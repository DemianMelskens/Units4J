...
public class KiloLiter implements Volume {

    private BigDecimal value;

    protected KiloLiter(final BigDecimal value) {
        this.value = value;
    }
    ...
    public String toString() {
        return String.format("%s Kiloliter", value);
    }

    @Override
    public MilliMeter3 toMilliMeter3() {
        return Volume.millimeter3(this.multiply(BigDecimal.valueOf(Math.pow(10, 9))).getValue());
    }

    @Override
    public CentiMeter3 toCentiMeter3() {
        return Volume.centimeter3(this.multiply(BigDecimal.valueOf(Math.pow(10, 6))).getValue());
    }

    @Override
    public DeciMeter3 toDeciMeter3() {
        return Volume.decimeter3(this.multiply(BigDecimal.valueOf(Math.pow(10, 3))).getValue());
    }

    @Override
    public Meter3 toMeter3() {
        return Volume.meter3(this.value);
    }

    @Override
    public DecaMeter3 toDecaMeter3() {
        return Volume.decameter3(this.divide(BigDecimal.valueOf(Math.pow(10, 3))).getValue());
    }

    @Override
    public HectoMeter3 toHectoMeter3() {
        return Volume.hectometer3(this.divide(BigDecimal.valueOf(Math.pow(10, 6))).getValue());
    }
    ...
    public Volume add(final Volume volume) {
        this.value = this.value.add(volume.toKiloLiter().getValue());
        return this;
    }

    @Override
    public Volume subtract(final Volume volume) {
        this.value = this.value.subtract(volume.toKiloLiter().getValue());
        return this;
    }
